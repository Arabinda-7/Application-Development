...
class AddSubFeatureActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (subFeatureId != null) {
            ...
            if (targetFeature == null && projectId != -1L) {
    ...
    private fun setupLogic() {
        ...
        btnDelete.setOnClickListener {
            ...
            if (!removedFromActive && projectId != -1L) {
                synchronized(DataManager.projects) {
                    DataManager.projects.find { it.timestamp == projectId }?.subFeatures?.remove(targetFeature)
                }
            }
    ...
    private fun getOtherFeatures(): List<ProjectFeature> {
        ...
        // Priority 2: Fallback to persistent storage
        val list = activeList ?: (if (projectId != -1L) synchronized(DataManager.projects) {
            DataManager.projects.find { it.timestamp == projectId }?.subFeatures
        } else null)
