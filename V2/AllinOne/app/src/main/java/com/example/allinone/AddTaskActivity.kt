...
class AddTaskActivity : BaseActivity() {
    ...
    private fun updateReminderUI() {
        ...
        existingTask?.let { old ->
            val task = DataManager.tasks.find { it.timestamp == old.timestamp } ?: old
    ...
    private fun saveTask() {
        val name = etName.text.toString().trim()
        if (name.isNotEmpty()) {
            // Find actual reference in DataManager to ensure updates persist after background reloads
            val task = existingTask?.let { old ->
                DataManager.tasks.find { it.timestamp == old.timestamp }
            } ?: Task(name, section = currentSection)
