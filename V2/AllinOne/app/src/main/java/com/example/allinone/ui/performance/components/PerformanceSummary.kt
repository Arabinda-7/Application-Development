...
fun PerformanceSummary(
    ...
    val workoutProgressSum = if (!data.workoutDetails.isNullOrEmpty()) {
        data.workoutDetails.sumOf { (it.progress.toDouble() / it.target.coerceAtLeast(1).toDouble() * 100.0) }
    } else {
        data.workoutsCompleted.toDouble() * 100.0
    }
    
    val habitProgressSum = data.habitsCompleted.toDouble() * 100.0
    
    val totalProgressSum = if (isWorkoutContext) workoutProgressSum else habitProgressSum + workoutProgressSum
    val overallPercent = if (effectiveTotalItems > 0) (totalProgressSum / effectiveTotalItems).toInt() else 0
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            if (currentStreak != null && currentStreak > 0) {
                ...
                Column(horizontalAlignment = Alignment.Start) {
                    ...
                    Text(text = "STREAK", fontSize = 8.sp, color = Color.Gray, fontWeight = FontWeight.Bold)
                }
            }
                    ...
                    Card(
                        ...
                        Row(
                            ...
                            Column {
                                ...
                                Text(
                                    text = "Personal Best Consistency",
                                    fontSize = 12.sp,
                                    color = Color.White
                                )
                            }
